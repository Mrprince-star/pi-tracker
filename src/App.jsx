...
function UnlockRing({ percent, size = 144, label = "UNLOCKED" }) {
  const stroke = 11;
  ...
  return (
    <div style={{ position: "relative", width: size, height: size, flexShrink: 0 }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
        ...
        <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 10.5, color: COLORS.textMuted, letterSpacing: "0.06em" }}>{label}</span>
      </div>
    </div>
  );
}
...
export default function PiTracker() {
  ...
  const [lang, setLang] = useState(() => {
    try {
      const saved = typeof window !== "undefined" && window.localStorage.getItem("pi-tracker-lang");
  ...
  function changeLang(code) {
    setLang(code);
    try {
      window.localStorage.setItem("pi-tracker-lang", code);
    } catch {
  ...
  const proFeatures = [
    { icon: "📈", title: t("feature_charts"), desc: t("feature_charts_desc") },
    { icon: "⬇️", title: t("feature_csv"), desc: t("feature_csv_desc") },
    { icon: "🔔", title: t("feature_reminders"), desc: t("feature_reminders_desc") },
    { icon: "🎨", title: t("feature_themes"), desc: t("feature_themes_desc") },
  ];
  ...
  const toolIndex = [
    { key: "tool-mining", icon: "🧮", title: t("tool_mining_title"), desc: t("tool_mining_desc") },
    { key: "tool-currency", icon: "💱", title: t("tool_currency_title"), desc: t("tool_currency_desc") },
    { key: "tool-role", icon: "🏅", title: t("tool_role_title"), desc: t("tool_role_desc") },
    { key: "tool-checklist", icon: "📋", title: t("tool_checklist_title"), desc: t("tool_checklist_desc", { done: checklistDone }) },
  ];

  const navItems = [
    { key: "about", label: t("navAbout"), icon: "ⓘ" },
    { key: "support", label: t("navSupport") + " & FAQ", icon: "✦" },
    { key: "privacy", label: t("navPrivacy"), icon: "🔒" },
    { key: "contact", label: t("navContact"), icon: "✉" },
  ];
  const tabItems = [
    { key: "tools", label: t("navTools"), icon: "▣" },
    { key: "history", label: t("navHistory"), icon: "▤" },
    { key: "support", label: t("navSupport"), icon: "✦" },
    { key: "more", label: t("navMore"), icon: "☰" },
  ];
    ...
    <div style={{ minHeight: "100vh", background: COLORS.bgPage, fontFamily: "'Inter', sans-serif", color: COLORS.text, maxWidth: 480, margin: "0 auto", position: "relative", overflowX: "hidden", paddingBottom: 64 }}>
      ...
      <div style={{ background: COLORS.ink, padding: "16px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <button onClick={goHome} aria-label="Go to dashboard" style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", padding: 4 }}>
          <div style={{ width: 26, height: 26, borderRadius: "50%", background: COLORS.gold, color: COLORS.ink, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Fraunces', serif", fontSize: 13, fontWeight: 700 }}>T</div>
          <span style={{ color: "white", fontFamily: "'Fraunces', serif", fontSize: 16, fontWeight: 600 }}>{t("appName")}</span>
        </button>
        <div style={{ width: 36 }} />
      </div>
        ...
        <div style={{ padding: "16px 16px 50px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <Card style={{ background: `linear-gradient(135deg, ${COLORS.ink} 0%, ${COLORS.navyLight} 100%)`, border: "none" }}>
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: 16.5, fontWeight: 600, color: "white", marginBottom: 6 }}>
              {username ? t("welcomeBack", { name: username }) : t("welcomeTitle")}
            </div>
          ...
          <Card style={{ display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
            <UnlockRing percent={unlockedPercent} label={t("unlocked")} />
            <div style={{ flex: "1 1 140px", minWidth: 0 }}>
              <Label>{t("totalHoldings")}</Label>
          ...
          <div>
            <Label style={{ paddingLeft: 2 }}>{t("quickTools")}</Label>
          ...
          <Card>
            ...
            <Field label={t("miningBalance")} value={mined} onChange={setMined} />
            <Field label={t("migratedBalance")} value={migrated} onChange={setMigrated} />
            <Field label={t("lockedBalance")} value={locked} onChange={setLocked} />
          ...
          {lockedNum > 0 && (
            <Card>
              <Label>{t("unlockOutlook")}</Label>
              <p style={{ fontSize: 11.5, color: COLORS.textMuted, margin: "-2px 0 14px", lineHeight: 1.5 }}>{t("unlockOutlookNote")}</p>
          ...
          <Card>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
              <Label style={{ marginBottom: 0 }}>{t("unlockReminder")}</Label>
              {!tipped && <span style={{ fontSize: 10.5, color: COLORS.gold, fontWeight: 700, letterSpacing: "0.04em" }}>{t("extra")}</span>}
            </div>
            {tipped ? (
              <div style={{ marginTop: 8 }}>
                <p style={{ fontSize: 12.5, color: COLORS.textMuted, margin: "0 0 12px" }}>{t("reminderNote")}</p>
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button onClick={handleSaveReminder} style={{ padding: "10px 16px", borderRadius: 12, border: "none", background: COLORS.ink, color: "white", fontWeight: 600, fontSize: 13, cursor: "pointer" }}>{t("save")}</button>
                </div>
                {savedReminder && <div style={{ marginTop: 10, fontSize: 12.5, color: COLORS.ink, fontWeight: 600 }}>📌 {t("reminderSetFor", { date: new Date(savedReminder).toLocaleDateString() })}</div>}
              </div>
            ) : (
              <p style={{ fontSize: 12.5, color: COLORS.textMuted, margin: 0, lineHeight: 1.5 }}>{t("reminderTipNote")}</p>
            )}
          </Card>

          <Card style={{ background: `linear-gradient(135deg, ${COLORS.ink} 0%, ${COLORS.navyLight} 100%)`, border: "none" }}>
            <Label style={{ color: COLORS.goldSoft }}>{t("supportApp")}</Label>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.82)", lineHeight: 1.55, margin: "0 0 14px" }}>{t("supportBody")}</p>
            ...
            {tipped ? (
              <div>
                <div style={{ textAlign: "center", padding: "10px", background: "rgba(255,255,255,0.1)", borderRadius: 12, fontSize: 13, fontWeight: 600, color: COLORS.goldSoft, marginBottom: 14 }}>💛 {t("thanksSupport")}</div>
                <div style={{ fontSize: 11.5, fontWeight: 700, letterSpacing: "0.05em", textTransform: "uppercase", color: "rgba(255,255,255,0.6)", marginBottom: 8 }}>{t("chooseTheme")}</div>
                ...
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.6)", margin: "8px 0 0" }}>{t("currentTheme", { name: THEMES[activeTheme].name })}</p>
              </div>
            ) : (
              <button onClick={() => setShowTipModal(true)} style={{ width: "100%", padding: "13px", borderRadius: 14, border: "none", background: COLORS.gold, color: COLORS.ink, fontWeight: 700, fontSize: 14, cursor: "pointer" }}>{t("tipButton")}</button>
            )}
          </Card>

          <div style={{ textAlign: "center", fontSize: 10.5, color: "#A8A28E", padding: "4px 12px 0" }}>{t("footerNote")}</div>
        </div>
      )}
      ...
      {page === "history" && (
        <div style={{ padding: "16px 16px 50px", display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          {history.length === 0 ? (
            <Card><p style={{ fontSize: 13, color: COLORS.textMuted, margin: 0, lineHeight: 1.6 }}>{t("noSnapshots")}</p></Card>
          ) : (
            <>
              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 4 }}>
                  <Label style={{ marginBottom: 0 }}>{t("historicalChart")}</Label>
                  {!tipped && <span style={{ fontSize: 10.5, color: COLORS.gold, fontWeight: 700, letterSpacing: "0.04em" }}>{t("extra")}</span>}
                </div>
                {tipped ? (
                  <div style={{ marginTop: 10 }}>
                    <MiniLineChart data={history} />
                    <p style={{ fontSize: 11.5, color: COLORS.textMuted, textAlign: "center", margin: "8px 0 0" }}>{t("chartCaption")}</p>
                  </div>
                ) : (
                  <p style={{ fontSize: 12.5, color: COLORS.textMuted, margin: "8px 0 0", lineHeight: 1.5 }}>{t("chartTipNote")}</p>
                )}
              </Card>

              <Card>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <Label style={{ marginBottom: 0 }}>{t("savedSnapshots")}</Label>
                  {tipped ? (
                    ...
                    <span style={{ fontSize: 10.5, color: COLORS.gold, fontWeight: 700, letterSpacing: "0.04em" }}>{t("csvExtra")}</span>
                  )}
                </div>
      ...
      {page === "about" && (
        <>
          <PageHeader title="About Tracker for Pioneers" onBack={goHome} />
          <StaticPage>
            <Card>
              <P>Tracker for Pioneers is a personal companion for Pioneers who want a clearer picture of their Pi journey — holdings, mining rate, role progress, and the Mainnet steps ahead.</P>
              <H>Where it started</H>
              <P>It began as a simple question: with so many Pioneers, why is there no single place to make sense of mining bonuses, unlock timelines, and the checklist everyone eventually has to complete? Tracker for Pioneers brings those pieces together in one calm, easy-to-read space, built independently using the tools Pi makes available to outside developers.</P>
              <H>What it does</H>
              <P>You enter numbers you already see in your own Pi app — balances, days mining, referral counts. Tracker for Pioneers turns them into clear visuals: an unlock ring on your Dashboard, plus four individual tools — a mining rate calculator, a currency converter, a role progress tracker, and a Mainnet checklist. Snapshots you save are kept under History so you can see how your numbers move over time.</P>
              <H>What it doesn't do</H>
              <P>Tracker for Pioneers never asks for your passphrase, seed phrase, or wallet credentials, and it has no technical ability to read your real balance, KYC status, or migration status. Those aren't available through any public or developer channel, by design, to protect Pioneers. Every figure you see is based on what you've typed in yourself, plus general, publicly known information about how the network works.</P>
              <H>Why it exists</H>
              <P>Many Pioneers have said the unlock and migration process feels confusing, and that mining bonuses are hard to estimate without doing the math by hand. Tracker for Pioneers won't change the official process, but it can make today's numbers easier to understand while you wait for tomorrow's.</P>
              <H>Independence</H>
              <P>Tracker for Pioneers is an independent, third-party companion built using the Pi SDK. It is not built, operated, or endorsed by the Pi Core Team, and isn't an official Pi Network product.</P>
            </Card>
          </StaticPage>
        </>
      )}
      ...
      {page === "support" && (
        <>
          ...
          <StaticPage>
            <Card>
              <H>Is my data safe?</H>
              <P>Yes. Your entered balances and tool inputs stay on your device. Tracker for Pioneers never asks for your passphrase or wallet credentials, and has no way to access your real account data, even if it wanted to.</P>
              ...
              <P>Tipping is a one-time, optional payment of 1 π, sent through your Pi Wallet using the official Pi SDK. It simply unlocks a few extra display features within Tracker for Pioneers — historical charts, CSV export, unlock reminders, and custom themes. It never unlocks access to anyone else's data.</P>
              <H>Will Tracker for Pioneers ever show ads?</H>
              <P>If Tracker for Pioneers is approved for the Pi Ad Network, it may show occasional full-screen ads at natural pause points, like after saving a snapshot — never ads that block core functionality. You'll never be asked to watch an ad to access your own entered data.</P>
        ...
        <>
          ...
          <StaticPage>
            <Card>
              ...
              <H>What we collect</H>
              <P>Tracker for Pioneers stores the figures you manually enter — balances, mining stats, role progress, and checklist completion — locally on your own device. If you choose to connect with Pi, we receive only your Pi username, for personalization, as permitted by the Pi SDK's username scope.</P>
              <H>What we never collect</H>
              <P>We never request, receive, or store your passphrase, private keys, or any wallet credentials. We have no technical ability to access your actual Pi balance, KYC status, or account status, regardless of what you enter into Tracker for Pioneers.</P>
              <H>Payments</H>
              <P>Optional tips are processed entirely through the Pi Wallet via the Pi SDK's payments scope. Tracker for Pioneers never handles your payment credentials directly, and cannot initiate a payment without your explicit on-screen confirmation through the Pi Wallet itself.</P>
              <H>Advertising</H>
              <P>Tracker for Pioneers is not currently part of the Pi Ad Network. If it's approved in the future, any ads shown will be delivered through Pi's own ad framework, governed by Pi Network's privacy practices, separate from the data you enter into Tracker for Pioneers.</P>
              <H>Third parties</H>
      ...
      {page === "contact" && (
        <>
          ...
          <StaticPage>
            <Card>
              <P>This page is for reporting bugs, suggesting features, asking questions about how Tracker for Pioneers works, or flagging anything that feels off. We built this as Pioneers ourselves, so genuine feedback actually shapes what gets built next.</P>
              <H>Email</H>
          ...
          <div style={{ position: "relative", width: "78%", maxWidth: 300, height: "100vh", maxHeight: "100vh", background: COLORS.ink, padding: "20px 16px", display: "flex", flexDirection: "column", boxShadow: "4px 0 24px rgba(0,0,0,0.25)", overflowY: "auto", boxSizing: "border-box" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 28 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, paddingLeft: 4 }}>
                <div style={{ width: 30, height: 30, borderRadius: "50%", background: COLORS.gold, color: COLORS.ink, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "'Fraunces', serif", fontSize: 14, fontWeight: 700 }}>T</div>
                <span style={{ color: "white", fontFamily: "'Fraunces', serif", fontSize: 17, fontWeight: 600 }}>{t("appName")}</span>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
              {[{ key: "dashboard", label: t("navDashboard"), icon: "◆" }, { key: "tools", label: t("navTools"), icon: "▣" }, { key: "history", label: t("navHistory"), icon: "▤" }, ...navItems].map((item) => (
            ...
            <div style={{ marginTop: 18, paddingTop: 16, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
              <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.06em", textTransform: "uppercase", color: "rgba(255,255,255,0.4)", marginBottom: 8, paddingLeft: 2 }}>{t("language")}</div>
              <div style={{ display: "flex", flexDirection: "column", gap: 2 }}>
                {Object.keys(LANGUAGES).map((code) => (
                  <button key={code} onClick={() => changeLang(code)}
                    style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "9px 12px", borderRadius: 10, border: "none", background: lang === code ? "rgba(201,162,75,0.15)" : "transparent", color: lang === code ? COLORS.goldSoft : "rgba(255,255,255,0.8)", fontSize: 13.5, fontWeight: lang === code ? 600 : 500, cursor: "pointer", textAlign: "left" }}>
                    <span>{LANGUAGES[code].native}</span>
                    {lang === code && <span style={{ fontSize: 12 }}>✓</span>}
                  </button>
                ))}
              </div>
            </div>
            ...
            <div style={{ marginTop: "auto", paddingTop: 20, borderTop: "1px solid rgba(255,255,255,0.1)" }}>
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.4)", lineHeight: 1.6 }}>Tracker for Pioneers v1.0<br />Not affiliated with the Pi Core Team.</div>
            </div>
          </div>
        </div>
      )}
      ...
      {showTipModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(16,24,43,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 80 }} onClick={() => setShowTipModal(false)}>
          <div onClick={(e) => e.stopPropagation()} style={{ background: "white", borderTopLeftRadius: 24, borderTopRightRadius: 24, padding: "24px 20px 28px", width: "100%", maxWidth: 480, boxSizing: "border-box" }}>
            ...
            <div style={{ fontFamily: "'Fraunces', serif", fontSize: 19, fontWeight: 600, marginBottom: 6 }}>{t("tipTheDev")}</div>
            <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.5, marginBottom: 18 }}>{t("tipModalBody")}</p>
            <div style={{ display: "flex", justifyContent: "space-between", padding: "12px 14px", background: COLORS.bgPage, borderRadius: 12, marginBottom: 18, fontSize: 13.5 }}>
              <span style={{ color: COLORS.textMuted }}>{t("amount")}</span><span style={{ fontWeight: 600, fontFamily: "'JetBrains Mono', monospace" }}>1.00 π</span>
            </div>
            <button onClick={handleTip} style={{ width: "100%", padding: "13px", borderRadius: 14, border: "none", background: COLORS.ink, color: "white", fontWeight: 600, fontSize: 14.5, cursor: "pointer", marginBottom: 8 }}>{t("confirmTip")}</button>
            <button onClick={() => setShowTipModal(false)} style={{ width: "100%", padding: "13px", borderRadius: 14, border: "none", background: "transparent", color: COLORS.textMuted, fontWeight: 500, fontSize: 14, cursor: "pointer" }}>{t("cancel")}</button>
          </div>
        </div>
      )}
